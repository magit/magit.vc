...
- User Option: magit-buffer-name-format
  ...
  - ~%v~
    ...
    The value the buffer is locked to, in parentheses, if it is the
    buffer is actually locked to its value.  An empty string
    otherwise.
  ...
  - ~%t~
    ...
    if ~magit-uniquify-buffer-name~ is non-nil an abbreviation of that.

  The value should always contain either ~%m~ or ~%M~, ~%v~ or ~%V~, and ~%t~.
  If ~magit-uniquify-buffer-name~ is non-nil, then the value must end
  with ~%t~.
...
is available from https://magit.vc/elpa/v1.  But note that v1.4.2 is
...
An Elpa archive featuring obsolete Magit v1.4.2 and its dependencies
is available from https://magit.vc/elpa/v1.
